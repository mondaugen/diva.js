...
class JSParser:
  # Class holding a path to a javascript to parse
  def __init__(self, path_to_js):
    self.path_to_js = path_to_js

  def set_path_to_js(self, path):
    self.path_to_js = path

  def get_js_lines(self):
      js_file = open(self.path_to_js)
      js_lines = [line.strip('\n') for line in js_file.readlines()]
  
      return js_lines
  
  def get_settings(self, var_name):
      js = [line.strip() for line in self.get_js_lines()]
  ...
  def get_functions(self, public=False):
      js = self.get_js_lines() 
